...
// TestMeasureTimingRandomizedOrder verifies the timing measurement helper
// runs repetitions and returns samples.
func TestMeasureTimingRandomizedOrder(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))
	defer srv.Close()
	...
	_, port := serverAddr(t, srv)
	checker := testChecker(t, srv)
	bundleA := httpx.NewTimingClient(checker)
	bundleB := httpx.NewTimingClient(checker)

	portStr := strconv.Itoa(port)
	reqs := []httpx.TimingRequest{
		{
			Label:   "baseline",
			Request: evidence.Request{Method: http.MethodGet, URL: "http://app.example.com:" + portStr + "/a"},
			Bundle:  bundleA,
		},
		{
			Label:   "payload",
			Request: evidence.Request{Method: http.MethodGet, URL: "http://app.example.com:" + portStr + "/b"},
			Bundle:  bundleB,
		},
	}

	samples, err := httpx.MeasureTiming(context.Background(), reqs, 3)
	if err != nil {
		t.Fatalf("MeasureTiming: %v", err)
	}
	// 2 requests * 3 repetitions = 6 samples
	if len(samples) != 6 {
	...
	if counts["baseline"] != 3 || counts["payload"] != 3 {
		t.Fatalf("label counts = %v, want baseline:3 payload:3", counts)
	}
}
