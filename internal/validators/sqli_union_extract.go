...
// unionExtractValid checks shape and the canary slot.
...
// plantCanary performs the setup write that stores
// the nonce. A 4xx/5xx means it could not be planted.
func plantCanary(
...
// readArms replays the extract and control arms and
// reports where the canary surfaced.
func readArms(
	ctx context.Context, bundle *httpx.ClientBundle,
	ev sqliUnionExtractEvidence, marker string, creds *authstate.Credentials,
...
// applyCreds appends the auth-state header credentials.
