...
exposes the exact vulnerable call at `POST /preview` (form field `url`).
...
```bash
cd examples/GHSA-4gp8-rjrq-ch6q/app
...
```bash
go run ./examples/GHSA-4gp8-rjrq-ch6q
```
...
A verified report (`protocol: http`, correlation id matched) proves the server
performed the blind out-of-band fetch. Requires outbound DNS for `*.localtest.me`.
