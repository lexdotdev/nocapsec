...
These are local reproductions for known advisories and focused bug-shaped harnesses. They validate that `nocapsec` can replay evidence against the target you run: either a pinned vulnerable package harness, the affected app version described in that example, or a narrow local app that models the vulnerable write/trigger/read path.
