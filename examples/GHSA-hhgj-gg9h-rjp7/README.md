...
```bash
go run ./examples/GHSA-hhgj-gg9h-rjp7
```

The example replays the traversal request and a benign control
(`/appearance/langs/en_US.json`). A verified report proves the canary marker is
present in the traversal response and absent from the control — arbitrary read out
of the appearance directory. (`/etc/passwd` is reachable by the same flaw via
